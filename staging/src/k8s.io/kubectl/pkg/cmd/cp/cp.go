...
func (o *CopyOptions) copyToPod(src, dest fileSpec, options *exec.ExecOptions) error {
	...
	if err := o.checkDestinationIsDir(dest); err == nil {
		...
		dest.File = dest.File.Join(src.File.Base())
	}
	...
	destFileDir := dest.File.Dir().String()
...
func (o *CopyOptions) copyFromPod(src, dest fileSpec) error {
	...
	prefix := src.File.Prefix().Clean().StripShortcuts()
	return o.untarAll(prefix.String(), src, dest, reader)
}

func makeTar(src, dest pathSpec, writer io.Writer) error {
...
func recursiveTar(srcBase, srcFile, destBase, destFile pathSpec, tw *tar.Writer) error {
	matchedPaths, err := srcBase.Join(srcFile).Glob()
	if err != nil {
		return err
	}
	for _, fpath := range matchedPaths {
		...
		if stat.IsDir() {
			...
			for _, f := range files {
				if err := recursiveTar(srcBase, srcFile.Join(newLocalPath(f.Name())), destBase, destFile.Join(newRemotePath(f.Name())), tw); err != nil {
					return err
				}
			}
			return nil
...
func (o *CopyOptions) untarAll(prefix string, src, dest fileSpec, reader io.Reader) error {
	...
	for {
		...
		destFileName := dest.File.Join(newRemotePath(header.Name[len(prefix):]))

		if !dest.File.IsRelative(destFileName) {
