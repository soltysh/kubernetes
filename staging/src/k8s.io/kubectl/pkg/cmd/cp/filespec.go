/*
Copyright 2021 The Kubernetes Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

package cp

import (
	"path"
	"path/filepath"
	"strings"
)

type fileSpec struct {
	PodName      string
	PodNamespace string
	File         pathSpec
}

type pathSpec interface {
	String() string
	Dir() pathSpec
	Base() pathSpec
	Clean() pathSpec
	Join(pathSpec) pathSpec
	Glob() (matches []string, err error)
	Prefix() pathSpec
	StripShortcuts() pathSpec
	// IsRelative returns true if dest is pointing outside the base directory, false otherwise.
	IsRelative(pathSpec) bool
}

// localPath represents a client-native path, which will differ based
// on the client OS, its methods will use path/filepath package which
// is OS dependant
type localPath struct {
	file string
}

func newLocalPath(fileName string) pathSpec {
	file := stripTrailingSlash(fileName)
	return localPath{file: file}
}

func (p localPath) String() string {
	return p.file
}

func (p localPath) Dir() pathSpec {
	return newLocalPath(filepath.Dir(p.file))
}

func (p localPath) Base() pathSpec {
	return newLocalPath(filepath.Base(p.file))
}

func (p localPath) Clean() pathSpec {
	return newLocalPath(filepath.Clean(p.file))
}

func (p localPath) Join(elem pathSpec) pathSpec {
	return newLocalPath(filepath.Join(p.file, elem.String()))
}

func (p localPath) Glob() (matches []string, err error) {
	return filepath.Glob(p.file)
}

func (p localPath) Prefix() pathSpec {
	return newLocalPath(stripLeadingSlash(p.file))
}

func (p localPath) StripShortcuts() pathSpec {
	p = (p.Clean()).(localPath)
	return newLocalPath(stripPathShortcuts(p.file))
}

func (p localPath) IsRelative(dest pathSpec) bool {
	relative, err := filepath.Rel(p.file, dest.String())
	if err != nil {
		return false
	}
	return relative == "." || relative == stripPathShortcuts(relative)
}

// remotePath represents always UNIX path, its methods will use path
// package which is always using `/`
type remotePath struct {
	file string
}

func newRemotePath(fileName string) pathSpec {
	// we assume remote file is a linux container but we need to convert
	// windows path separators to unix style for consistent processing
	file := strings.ReplaceAll(stripTrailingSlash(fileName), `\`, "/")
	return remotePath{file: file}
}

func (p remotePath) String() string {
	return p.file
}

func (p remotePath) Dir() pathSpec {
	return newRemotePath(path.Dir(p.file))
}

func (p remotePath) Base() pathSpec {
	return newRemotePath(path.Base(p.file))
}

func (p remotePath) Clean() pathSpec {
	return newRemotePath(path.Clean(p.file))
}

func (p remotePath) Join(elem pathSpec) pathSpec {
	return newRemotePath(path.Join(p.file, elem.String()))
}

func (p remotePath) Glob() (matches []string, err error) {
	panic("unsupported operation: remotePath.Glob")
}

func (p remotePath) Prefix() pathSpec {
	return newLocalPath(stripLeadingSlash(p.file))
}

func (p remotePath) StripShortcuts() pathSpec {
	p = (p.Clean()).(remotePath)
	return newLocalPath(stripPathShortcuts(p.file))
}

func (p remotePath) IsRelative(dest pathSpec) bool {
	panic("unsupported operation: remotePath.IsRelative")
}

// strips trailing slash (if any) both unix and windows style
func stripTrailingSlash(file string) string {
	if len(file) == 0 {
		return file
	}
	if file != "/" && strings.HasSuffix(string(file[len(file)-1]), "/") {
		return file[:len(file)-1]
	}
	return file
}

func stripLeadingSlash(file string) string {
	// tar strips the leading '/' and '\' if it's there, so we will too
	return strings.TrimLeft(file, `/\`)
}

// stripPathShortcuts removes any leading or trailing "../" from a given path
func stripPathShortcuts(p string) string {
	newPath := p
	trimmed := strings.TrimPrefix(newPath, "../")

	for trimmed != newPath {
		newPath = trimmed
		trimmed = strings.TrimPrefix(newPath, "../")
	}

	// trim leftover {".", ".."}
	if newPath == "." || newPath == ".." {
		newPath = ""
	}

	if len(newPath) > 0 && string(newPath[0]) == "/" {
		return newPath[1:]
	}

	return newPath
}
